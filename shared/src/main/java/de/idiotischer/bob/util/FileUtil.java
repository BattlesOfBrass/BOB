...
public class FileUtil {
    ...
    public static Path getFlag(Scenario scenario, Country c) {
        Ideology i = c.getRulingIdeology();
        Ideology i2 = c.getStartIdeology();

        String suffix = "";

        //if (i2 == null && i != null) suffix = i.abbreviation();
        //if(i2 != null && i != null && i2.abbreviation().equals(i.abbreviation())) suffix = "";

        //TODO: idk if this is right, if not use the above
        if (i != null && (i2 == null || !i2.abbreviation().equals(i.abbreviation()))) suffix = i.abbreviation();

        String file = c.getAbbreviation() + (suffix.isEmpty() ? "" : "-" + suffix) + ".png";
        Path countryFlagsDir = getDefaultFlagsDir().resolve(c.getAbbreviation()).resolve(file);
        Path countryFlagsDirSc = getDefaultFlagsDir(scenario).resolve(c.getAbbreviation()).resolve(file);
